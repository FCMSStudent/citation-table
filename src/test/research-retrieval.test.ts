...
describe("Research Retrieval Response Validation", () => {
  it("should validate ResearchResponse structure", () => {
    ...
    expect(response.total_papers_searched).toBe(0);
  });

  it("should handle normalized query in response", () => {
    const response: ResearchResponse = {
      results: [],
      query: "effects of treatment",
      normalized_query: "reported outcomes associated with treatment",
      total_papers_searched: 10,
      openalex_count: 5,
      semantic_scholar_count: 5,
    };
    
    expect(response.normalized_query).toBeTruthy();
    expect(response.normalized_query).toContain("reported outcomes");
    expect(response.openalex_count).toBe(5);
    expect(response.semantic_scholar_count).toBe(5);
  });
  ...
  it("should handle error responses", () => {
    ...
    expect(errorResponse.results.length).toBe(0);
  });
  ...
  it("should validate citation count from semantic scholar", () => {
    ...
    expect(study.source).toBe("semantic_scholar");
  });

  it("should validate multiple identifiers in citation", () => {
    const study: StudyResult = {
      study_id: "test-789",
      title: "Multi-ID Study",
      year: 2023,
      study_design: "RCT",
      sample_size: 150,
      population: "Test population",
      ...
      citation: {
        ...
        formatted: "Test et al. (2023). Multi-ID Study. Journal."
      },
      abstract_excerpt: "Test abstract",
      preprint_status: "Peer-reviewed",
      review_type: "None",
    ...
    expect(study.citation.openalex_id).toBeTruthy();
    expect(study.citation.formatted).toBeTruthy();
  });
  ...
  it("should validate review type classification", () => {
    const metaAnalysis: StudyResult = {
      ...
      population: null,
      outcomes: [{
        outcome_measured: "Pooled outcome",
        key_result: "Combined result",
        citation_snippet: "Meta-analysis shows"
      }],
      citation: {
        doi: "10.1234/meta",
        pubmed_id: null,
      ...
      abstract_excerpt: "Meta-analysis abstract",
      preprint_status: "Peer-reviewed",
    ...
    expect(metaAnalysis.study_design).toBe("review");
  });

  it("should handle null sample size and population", () => {
    const study: StudyResult = {
      study_id: "test-null",
      title: "Study with Nulls",
      year: 2023,
